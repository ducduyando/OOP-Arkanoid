...
public class Chainsaw extends Actor {
    ...
    public Chainsaw(Texture sawTexture, Texture targetTexture, float x, float y, boolean isVertical) {
    ...
    public boolean isChainsawSkillFinished() {
        return isChainsawSkillFinished;
    }
    ...
    public void act(float delta) {
        ...
        if (!isVertical) {
            moveBy(HORIZONTAL_CHAINSAW_SPEED * delta * direction, 0);
        } else {
            moveBy(0, VERTICAL_CHAINSAW_SPEED * delta * direction);
        }
        if (getX() >= SCREEN_WIDTH || getX() <= -BOSS3_SKILL_1B_WIDTH
            || getY() >= UP_BOUNDARY || getY() <= -BOSS3_SKILL_1B_HEIGHT) {
            this.remove();
            isChainsawSkillFinished = true;
            return;
        }
        hitBox.setPosition(getX(), getY());
        stateTimer += delta;
        currentFrame = chainsawSkillAnimation.getKeyFrame(stateTimer, true);
    }
